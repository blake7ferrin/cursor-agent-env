...
test('POST /mcp initialize returns JSON-RPC server capabilities', async () => {
  const res = await request(app)
    .post('/mcp')
    .set(auth)
    .set('Content-Type', 'application/json')
...
test('POST /mcp tools/list returns JSON-RPC tools array', async () => {
  const res = await request(app)
    .post('/mcp')
    .set(auth)
    .set('Content-Type', 'application/json')
...
test('POST /mcp tools/call housecall.get_config returns JSON-RPC tool result', async () => {
  const res = await request(app)
    .post('/mcp')
    .set(auth)
    .set('Content-Type', 'application/json')
  ...
  assert.ok(typeof res.body?.result?.content?.[0]?.text === 'string');
});
