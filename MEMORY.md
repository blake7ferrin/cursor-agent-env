...
- **Housecall Pro integration (2026-02-28):** Client in `bridge/housecall-pro.js`. API base: `https://api.housecallpro.com` (no /v1 prefix). Auth: Bearer token via `HCP_API_KEY` or `HOUSECALL_PRO_ACCESS_TOKEN`. `npm run create-estimate` creates HVAC estimate for Tony Stark (or args) with 45% margin. Estimate payload: `customer_id`, `options: [{ name, total_amount (cents), line_items }]`.
