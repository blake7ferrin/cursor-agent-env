...
- Session: Extended runtime catalog behavior into `POST /estimator/estimate` and Housecall export estimate-building path, added estimate compatibility test for adapted ingest catalog, and verified full test suite passing.
- Session: Added Housecall API key alias support (`HCP_API_KEY` alongside `HOUSECALL_PRO_API_KEY`), added tests for alias precedence, and confirmed the current running shell still needs secret injection/restart to detect newly added keys.
- Session: Removed `.cursor/environment.json` from the branch so cloud agents use the newly saved Cursor environment secrets instead of repo-level environment overrides.
- Session: Fixed PR merge issues by merging origin/main into cursor/hvac-pricing-agent-3304, resolving conflicts in MEMORY/docs/import/server files, rerunning bridge tests (31/31 pass), and pushing merge commit ee9c91b.
